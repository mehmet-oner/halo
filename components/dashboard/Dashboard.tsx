...
export default function Dashboard({ displayName, email, onSignOut }: DashboardProps) {
  ...
  const addPollOption = () => {
    setNewPollOptions((prev) => (prev.length < 6 ? [...prev, ''] : prev));
  };
      ...
      <main className="mx-auto flex w-full max-w-xl flex-col gap-6 px-5 pb-36 pt-6">
        <section className="rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-sm backdrop-blur">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-medium text-slate-900">Quick status</h2>
              <p className="text-sm text-slate-500">
                Share where you are or what you are up to
              </p>
            </div>
            <button
          ...
          {showStatusPicker && (
            <div className="space-y-4 rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-inner">
              ...
              <div>
                <button
                  onClick={() => setShowCustomStatus((prev) => !prev)}
        ...
        <section className="rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-sm backdrop-blur">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Check ins from everyone in {currentGroup.name}
              </p>
            </div>
            <button className="flex items-center gap-2 rounded-full border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 transition hover:border-slate-400 hover:bg-slate-100/70">
              <Users size={16} />
              <span>Invite</span>
            </button>
          </div>
          ...
          <div className="space-y-3">
            {currentGroup.members.map((member) => {
              ...
              const currentStatus: MemberStatus | null =
