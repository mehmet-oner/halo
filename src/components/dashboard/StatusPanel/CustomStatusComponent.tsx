...
type CustomStatusComponentProps = {
  ...
  setShowCustomStatus: Dispatch<SetStateAction<boolean>>;
...
export default function CustomStatusComponent({
  ...
  const handleSave = async () => {
    if (!allowUpdates || isUploadingImage) return;

    const trimmedMessage = customMessage.trim();
    if (!trimmedMessage && !statusImage) return;

    try {
      await onSubmit({
        message: trimmedMessage || "Custom status",
        image: statusImage,
        emoji: "💬",
      });
      resetForm();
  ...
  return (
    <div>
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-dashed border-slate-300 px-4 py-3 text-sm font-medium text-slate-600 transition hover:border-slate-400 hover:bg-slate-100/60 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400"
      >
        ...
        <div className="mt-4 space-y-4 rounded-xl border border-slate-200 bg-white/70 p-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={
                (!customMessage.trim() && !statusImage) ||
                isUploadingImage ||
                !allowUpdates
              }
